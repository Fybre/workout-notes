...
export function UnitProvider({ children }: UnitProviderProps) {
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        ...
        console.warn("[UnitContext] Failed to load preferences:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const setWeightUnit = async (unit: "kg" | "lbs") => {
    try {
      ...
      console.warn("[UnitContext] Failed to save weight unit:", error);
    }
  };
  ...
  const setDistanceUnit = async (unit: "km" | "miles") => {
    try {
      ...
      console.warn("[UnitContext] Failed to save distance unit:", error);
    }
  };
  ...
  const setWeightIncrement = async (increment: number) => {
    try {
      ...
      console.warn("[UnitContext] Failed to save weight increment:", error);
    }
  };
